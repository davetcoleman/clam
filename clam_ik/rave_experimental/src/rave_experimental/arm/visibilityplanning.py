...
class VisibilityGrasping(metaclass.AutoReloader):
    ...
    def __init__(self,env):
        ...
        self.trajectorylog = []
        self.graspoffset = 0
        self.target_name  = "frootloops" # original target from OpenRAVE developers
        								 # name of body to target
        								 
        self.loadscene("rgbCamera", robotname="pdArm", showsensors=False)
        
    def loadscene(self, sensorname,robotname=None,showsensors=True,usecameraview=True):
    	#scenefilename,sensorname,robotname=None,showsensors=True,usecameraview=True):
        self.target = None
        self.robot = None
        self.robotreal = None
    ...
    def gettarget(self,orenv):
        with orenv:
            ...
            targets = [b for b in bodies if b.GetName().find(self.target_name) >=0 ]
            if len(targets) > 0:
                return targets[0]
        return None
